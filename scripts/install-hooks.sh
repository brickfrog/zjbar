...
uninstall() {
  ...
  # Remove only zjbar hook entries (those matching our script path)
  local tmp
  tmp=$(mktemp)
  ...
  ' "$SETTINGS" > "$tmp"
  mv "$tmp" "$SETTINGS"
  echo "Uninstalled zjbar hooks from $SETTINGS"
}
...
install() {
  ...
  # First uninstall any existing zjbar hooks to avoid duplicates
  ...
  ' "$SETTINGS" > "$tmp"
  mv "$tmp" "$SETTINGS"
  echo "Installed zjbar hooks into $SETTINGS"
